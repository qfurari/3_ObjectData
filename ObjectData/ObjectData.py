...
class ObjectData(OpenRTM_aist.DataFlowComponentBase):
    ...
    def generate_image(self, amplitude):
        ...
        num_sides = shapes[min(shape_index, len(shapes) - 1)]

        blue = min(255, int((amplitude / 10000) * 255)) 
        green = 0
        red = min(255, int(((10000 - amplitude) / 10000) * 255))
...
def MyModuleInit(manager):
    profile = OpenRTM_aist.Properties(defaults_str='implementation_id=VoiceToImageConverter')
    manager.registerFactory(profile,
                            VoiceToImageConverter,
                            OpenRTM_aist.Delete)
    
    comp = manager.createComponent("VoiceToImageConverter")
